...
gj_data <- read.csv("data/USW00023066.csv")
fc_data <- read.csv("data/USC00053005.csv")
```
...
gj.max.lm <- lm(TMAX ~ DATE, data = gj_data)
summary(gj.max.lm)
```

```{r, linear model Grand Junction minimum temp}
gj.min.lm <- lm(TMIN ~ DATE, data = gj_data)
summary(gj.min.lm)
```

```{r, linear model Fort Collins maximum temp}
fc.max.lm <- lm(TMAX ~ DATE, data = fc_data)
summary(fc.max.lm)
```

```{r, linear model Fort Collins minimum temp}
fc.min.lm <- lm(TMIN ~ DATE, data = fc_data)
summary(fc.min.lm)
```
